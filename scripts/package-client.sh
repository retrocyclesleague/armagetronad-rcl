...
# The AI player definitions are also generated by configure. Keep the runtime
# file, rather than only its source template, in every client archive.
if test -f "${BUILD_ROOT}/config/aiplayers.cfg"; then
  cp "${BUILD_ROOT}/config/aiplayers.cfg" \
    "${PACKAGE_ROOT}/config/aiplayers.cfg"
fi
